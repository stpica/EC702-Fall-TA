...
% prod function: f(k,l)=k^alpha*l^(1-alpha)
...
C_ss1=A*K_ss1^alpha*L_ss1^(1-alpha)+(1-delta)*K_ss1-K_ss1; %consumption

%exact SS quantities - lower alpha
L_ss2=((1/psi)*(1-alpha)/(1-alpha*delta/(1/beta-1+delta)))^(1/(epsilon+1)); %labor
K_ss2=(Ap*alpha/(1/beta-1+delta))^(1/(1-alpha))*L_ss2; %capital
R_ss2=Ap*alpha.*K_ss2.^(alpha-1).*L_ss2.^(1-alpha)+(1-delta); %interest rate
W_ss2=Ap*(1-alpha).*K_ss2.^(alpha).*L_ss2.^(-alpha); %wage
C_ss2=Ap*K_ss2^alpha*L_ss2^(1-alpha)+(1-delta)*K_ss2-K_ss2; %consumption
...
Gmax2=Ap*k.^alpha.*l.^(1-alpha)+(1-delta)*k;
...
matrix_uxy2=log(Ap*k.^alpha.*l.^(1-alpha)+(1-delta)*k-kp)-psi*l.^(1+epsilon)/(1+epsilon);
...
for vfit = 1:vfmaxit
    ...
    %emphasize that lprimeind and kprimeind ARE the policy functions...
...
Cpol2 = Ap*K0.^alpha.*Lpol2.^(1-alpha)+(1-delta)*K0-Kpol2;
R2 = Ap*alpha.*K0.^(alpha-1).*Lpol2.^(1-alpha)+(1-delta); %interest rate
W2 = Ap*(1-alpha).*K0.^(alpha).*Lpol2.^(-alpha); %wage
...
legend('Low TFP','High TFP','Location','best')
legend boxoff
...
legend('Low TFP','High TFP','45 degree line','Location','best')
legend boxoff
set(gca,'FontSize',fsizenum)
...
legend('Capital','Low TFP SS','High TFP SS','Location','best')
legend boxoff
set(gca,'FontSize',fsizenum)
...
legend('Labor','Low TFP SS','High TFP SS','Location','best')
legend boxoff
set(gca,'FontSize',fsizenum)
...
legend('Consumption','Low TFP SS','High TFP SS','Location','best')
legend boxoff
set(gca,'FontSize',fsizenum)
...
legend('Wage','Low TFP SS','High TFP SS','Location','best')
legend boxoff
set(gca,'FontSize',fsizenum)
...
legend('Interest Rate','Low TFP SS','High TFP SS','Location','best')
legend boxoff
set(gca,'FontSize',fsizenum)
